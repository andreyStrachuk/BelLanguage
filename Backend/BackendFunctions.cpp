...
// return register --- ax
static const char gReturnReg[] = "ax";

static int gIfCounter = 0;
static int gWhileCounter = 0;
static int gGlobalFree = 0;
...
void AssembleFunctions (TreeNode *node, FILE *file) {
    ...
    tableOfVars.free = gGlobalFree;
    tableOfVars.startAddr = gGlobalFree;
    tableOfVars.size = 0;
...
int VarsTableConstruct (VarsTable *table, const int varsNum) {
    assert (table);

    table->free = 0;
    table->vars = (Variable *)calloc (varsNum, sizeof (Variable));

    if (table->vars == nullptr)
        return NULLADDR;
...
int VarsTableDestruct (VarsTable *table) {
    assert (table);
